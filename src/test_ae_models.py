...
artifact = run.use_artifact(
    f'robot-anomaly-detection/anomaly_detection/ae-e{args.load_epoch}:latest',
    type='ae-models'
)
artifact_dir = artifact.download()
model_path = os.path.join(
    artifact_dir, 
    f'ae_v1_e{args.load_epoch}.ckpt'
)
...
for threshold in np.linspace(0.1, 1.5, 15):
    ...
    print(f'Autoencoder -- OOD rate: {ood_success_rate}, IND rate: {ind_success_rate}')

classification_results["ae"] = success_rates
...
for key in classification_results.keys():
    ...
    plot_label = f"nf-{key}" if key != "ae" else "ae"
