...
# Download artifacts for all model types
artifact = run.use_artifact(
    f'robot-anomaly-detection/anomaly_detection/vanilla-vae-e{args.load_epoch}:latest',
    type='vae-models'
)
artifact_dir = artifact.download()
model_path = os.path.join(
    artifact_dir, 
    f'vae_v1_e{args.load_epoch}.ckpt'
)
...
for threshold in np.linspace(0.1, 1.5, 15):
    ...
    print(f'VAE -- OOD rate: {ood_success_rate}, IND rate: {ind_success_rate}')

classification_results["vae"] = success_rates
...
for key in classification_results.keys():
    success_rates = classification_results[key]
    ood_success_rate = [success[0] for success in success_rates]
    ind_success_rate = [success[1] for success in success_rates]
    plot_label = key if key == "vae" or key == "ae" else f"nf-{key}"
    ax[0].plot(np.linspace(0.1, 1.5, 15), ood_success_rate, label=plot_label)
    ax[1].plot(np.linspace(0.1, 1.5, 15), ind_success_rate, label=plot_label)
